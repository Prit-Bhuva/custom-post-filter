...
class CustomPostFilter
{
    ...
    {
        // Retrieve selected post types from the options
        $selectedPostTypes = get_option('custom_post_types', []);

        // If no post types are selected, fallback to defaults
    ...
    public function getPostWiseTaxonomies()
    {
        ...
        if (!empty($all_post_types)) {
            foreach ($all_post_types as $singlePostType) {
                ...
                $postType_taxonomy[$singlePostType] = isset($this->getTaxonomySlugsByPostType($singlePostType)[0])
                    ? $this->getTaxonomySlugsByPostType($singlePostType)[0]
                    : '';
            }
        }
    ...
    /**
     * Retrieves the taxonomy by the selected post type.
     *
     * @param array $postTypes An array of post types.
     * @param array $postTerms An array of post terms.
